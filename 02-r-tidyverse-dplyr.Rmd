...
## La función ```select()```
La función [select()](https://dplyr.tidyverse.org/reference/select.html) selecciona variables con base en sus nombres.

El siguiente bloque de código selecciona un conjunto de variables del data frame de datos de COVID-19 en Costa Rica y guarda el resultado en un nuevo data frame.

```{r}
# Selección, renombramiento y conversión de columnas 
covid_cols_seleccionadas <-
  covid %>%
...
slice_sample(covid_cols_seleccionadas, n = 5)
```